...
/**
 * Waves — fundo animado (canvas + ruído de Perlin + interação com o cursor).
 * Adaptado do React Bits: https://www.reactbits.dev/backgrounds/waves
 * Código-fonte: https://github.com/DavidHDev/react-bits
 */
import "./waves.css";
...
class Noise {
  ...
  constructor(seed = 0) {
    ...
    this.p = [
      ...
      39, 253, 19, 98, 108, 110, 79, 113, 224, 232, 178, 185, 112, 104, 218, 246, 97, 228, 251, 34,
  ...
  perlin2(x: number, y: number) {
    ...
    const g = this.gradP;
    const p = this.perm;
    const n00 = g[X + p[Y]]!.dot2(x, y);
    const n01 = g[X + p[Y + 1]]!.dot2(x, y - 1);
    const n10 = g[X + 1 + p[Y]]!.dot2(x - 1, y);
    const n11 = g[X + 1 + p[Y + 1]]!.dot2(x - 1, y - 1);
    const u = this.fade(x);
...
export default function Waves({
  ...
  const boundingRef = useRef<{ width: number; height: number }>({
    width: 0,
    height: 0,
  });
  ...
  useEffect(() => {
    ...
    function setLines() {
      ...
      const { xGap: gx, yGap: gy } = configRef.current;
      const totalLines = Math.ceil(oWidth / gx);
      const totalPoints = Math.ceil(oHeight / gy);
    ...
    function movePoints(time: number) {
      ...
      lines.forEach((pts) => {
        pts.forEach((p) => {
          ...
          p.cursor.x = Math.min(maxM, Math.max(-maxM, p.cursor.x));
          p.cursor.y = Math.min(maxM, Math.max(-maxM, p.cursor.y));
        });
      });
    }
